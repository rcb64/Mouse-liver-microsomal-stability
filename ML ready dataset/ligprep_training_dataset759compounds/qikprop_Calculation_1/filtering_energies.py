...
structures = structure.StructureReader(r"C:\Users\kmml910\OneDrive - AZCollaboration\Documents\Coding\Mouse_microsomal_stability\ML ready dataset\better ligprep_training_dataset759compounds\qikprop_Calculation_1\qikprop_Calculation_1-out.maegz")
...
with structure.StructureWriter('single_conformers_training.mae') as writer:
    for conformer, _ in conformer_dict.values():
        writer.append(conformer)
        
with structure.StructureWriter('single_conformers_training.sdf') as writer:
    for conformer, _ in conformer_dict.values():
        writer.append(conformer)
        
with structure.StructureWriter('single_conformers_training.csv') as writer:
    for conformer, _ in conformer_dict.values():
        writer.append(conformer)
